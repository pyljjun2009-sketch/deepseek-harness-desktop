...
Windows x64 安装包：[查看最新发布版](https://github.com/pyljjun2009-sketch/deepseek-harness-desktop/releases/latest)。安装包未签名；安装前可对照 Release 发布说明中的 SHA-256 校验值。
...
5. 运行时异常时先自动停用可选 Sub2API 插件，再关闭省 token 配置重试；若候选仍失败，则同时切回上一稳定版本及其数据槽。失败候选槽保留以便人工找回期间产生的会话，自动回滚不会合并这些数据。
...
## 按需使用 Sub2API Personal

此集成默认关闭，不替换工作台的默认聊天模型。它将本地 `dsh-sub2api-personal` 作为一个 DSH Agent 工具随应用打包；仅在启用后，Agent 才能按需调用。使用前需在本机安装和配置 `sub2api-personal` 客户端，使 `%USERPROFILE%\plugins\sub2api-personal\scripts\sub2api.ps1` 存在，并安装 PowerShell 7。桌面版不会捆绑或复制本地账号、密钥或客户端配置。

在“运行时与升级 → Sub2API Personal”填写 `pwsh.exe` 的绝对路径，点击“读取账号”，只勾选允许 Agent 使用的账号，再启用工具。启用时会预检插件能否随 DSH Web 启动；若插件引起运行时故障，会自动停用并重新启动稳定配置。模型调用可能消耗所选账号额度；请在任务中明确指定要使用的账号。此工具不是自动路由，也不会暗中切换默认模型。实现边界与故障策略见 [docs/adr/0006-optional-sub2api-tool.md](docs/adr/0006-optional-sub2api-tool.md)。
