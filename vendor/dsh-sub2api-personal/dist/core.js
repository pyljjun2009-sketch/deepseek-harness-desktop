...
function isValidProfileName(value) { return PROFILE_NAME_PATTERN.test(value); }
function configuredProfileAllowlist() {
    ...
    if (names.some((value) => !isValidProfileName(value)))
        throw new Error('SUB2API_PERSONAL_ALLOWED_PROFILES contains an invalid profile name.');
    return new Set(names);
}
function normalizeProfileList(value, allowlist) {
    let parsed;
    try {
        parsed = JSON.parse(value);
    }
    catch {
        throw new Error('The local client returned an invalid profile list.');
    }
    if (!Array.isArray(parsed))
        throw new Error('The local client returned an invalid profile list.');
    const profiles = parsed.map((entry) => {
        if (!isJsonObject(entry) || typeof entry.name !== 'string' || !isValidProfileName(entry.name))
            throw new Error('The local client returned an invalid profile list.');
        return { name: entry.name };
    });
    if (new Set(profiles.map(({ name }) => name)).size !== profiles.length)
        throw new Error('The local client returned duplicate profile names.');
...
function clip(value, maxChars = MAX_OUTPUT_CHARS) {
    if (maxChars <= 0)
        return '';
...
function appendCapped(value, chunk) {
    if (value.length >= MAX_CAPTURE_CHARS)
        return value;
    return value + chunk.slice(0, MAX_CAPTURE_CHARS - value.length);
}
async function runPowerShell(args, timeoutSec, signal) {
    if (signal?.aborted)
        return { exitCode: 1, stdout: '', stderr: 'The Sub2API request was cancelled by the host.', timedOut: false };
    const script = clientPath();
    if (!script)
        return { exitCode: 1, stdout: '', stderr: 'SUB2API_PERSONAL_CLIENT_PATH is not set and no user home directory is available.', timedOut: false };
    try {
        await access(script);
    }
    catch {
...
export async function executeSub2Api(args, signal) {
    const profileName = args.profileName ?? '';
    const invalid = (message) => ({ ok: false, action: args.action, profileName, data: '', error: message, exitCode: 2 });
    if (['test_profile', 'models', 'invoke'].includes(args.action) && !profileName)
        return invalid('profileName is required for this action.');
    if (profileName && !isValidProfileName(profileName))
        return invalid('profileName must match a configured Sub2API profile name (1–64 letters, numbers, dot, underscore, or hyphen; it must start with a letter or number).');
    if (args.action === 'invoke' && !args.path)
        return invalid('path is required for invoke.');
    if (args.action === 'invoke' && !['chat/completions', 'responses'].includes(args.path))
        return invalid('path must be chat/completions or responses.');
    let temporaryDirectory = '';
    try {
        const allowlist = configuredProfileAllowlist();
        if (allowlist !== undefined && profileName && !allowlist.has(profileName))
            return invalid(`Profile "${profileName}" is not allowed for this process.`);
        if (allowlist !== undefined && args.action === 'usage' && !profileName)
            return invalid('profileName is required for usage when SUB2API_PERSONAL_ALLOWED_PROFILES is configured.');
        const timeoutSec = args.timeoutSec ?? 120;
        if (!Number.isInteger(timeoutSec) || timeoutSec < 1 || timeoutSec > 900)
            return invalid('timeoutSec must be an integer from 1 through 900.');
        const command = [];
        switch (args.action) {
            case 'list_profiles':
                command.push('list-profiles');
                break;
            case 'test_profile':
                command.push('test-profile', '-ProfileName', profileName);
                break;
            case 'models':
                command.push('models', '-ProfileName', profileName);
                break;
            case 'usage': {
                const days = args.days ?? 30;
                if (!Number.isInteger(days) || days < 1 || days > 3650)
                    return invalid('days must be an integer from 1 through 3650.');
                command.push('usage', '-Days', String(days));
                if (profileName)
                    command.push('-ProfileName', profileName);
                break;
            }
            case 'invoke': {
                if (!args.bodyJson)
                    return invalid('bodyJson is required for invoke requests.');
                let requestBody;
                try {
                    requestBody = JSON.parse(args.bodyJson);
                }
                catch {
                    return invalid('bodyJson must be valid JSON.');
                }
                if (!isJsonObject(requestBody))
                    return invalid('bodyJson must be a JSON object.');
                if ('stream' in requestBody && typeof requestBody.stream !== 'boolean')
                    return invalid('bodyJson.stream must be a boolean when provided.');
                if ((requestBody.stream === true) !== (args.stream === true))
                    return invalid('stream must match bodyJson.stream exactly; set both to true for SSE or omit/false for a non-streaming response.');
                if (Buffer.byteLength(args.bodyJson, 'utf8') > MAX_BODY_BYTES)
                    return invalid('bodyJson exceeds the 1 MiB safety limit.');
        ...
        if (temporaryDirectory) {
            ...
            catch { /* best effort after returning a structured failure */ }
        }
    }
}
